...
function camelCase(item1) {
  let a = /[^\w]/g;
  let item = item1.split(a);
  let str = "";
  if (item.length > 1) {
    for (let i = 0; item.length > i; i++) {
      if (i == 0) {
        str += item[0][0].toLocaleUpperCase() + item[0].slice(1);
      } else {
        str +=
          item[i][0].toLocaleUpperCase() + item[i].toLocaleLowerCase().slice(1);
      }
    }
  } else {
    str = item1[0].toLocaleUpperCase() + item1.slice(1);
  }

  return str;
}

console.log(camelCase("JavaScript coding ChAlLenge"));
console.log(camelCase("JavaScript-coding-challenge"));
console.log(camelCase("javaScriptCodingChallenge"));
